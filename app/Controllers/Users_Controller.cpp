...
namespace App::Controllers {
    ...
    void Users_Controller::RegisterRoutes(crow::App<CORS>& app) {
        ...
        CROW_ROUTE(app, "/set/blocked/user/id").methods(crow::HTTPMethod::Post)([JWT, AES](const crow::request& req) {
            auto body = crow::json::load(req.body);

            if (!body ||
                !body.has("id") ||
                !body.has("token") ||
                !body.has("blocked_user_id")
                ) {
            ...
            try {
                ...
                std::string encrypted_blocked_user_id = body["blocked_user_id"].s();
                std::string decrypted_blocked_user_id = AES.Decrypt(encrypted_blocked_user_id);

                client.sadd(decrypted_user_id, { decrypted_blocked_user_id });
                client.sync_commit();
        ...
        CROW_ROUTE(app, "/get/blocked/user/ids").methods(crow::HTTPMethod::Post)([JWT,AES](const crow::request& req) {
            auto body = crow::json::load(req.body);
        ...
        CROW_ROUTE(app, "/delete/blocked/user/id").methods(crow::HTTPMethod::Post)([JWT, AES](const crow::request& req) {
            auto body = crow::json::load(req.body);
            ...
            if (!body ||
                ...
                !body.has("token") ||
                !body.has("blocked_user_id")) {
                return crow::response(400, "Error: 1"); // Missing Payload Info
            }

            std::string JWT_Client_Side_Token = body["token"].s();

            if (!JWT.Authenticate_Claims(JWT_Client_Side_Token)) {
                return crow::response(400, "Error: 2"); // Invalid token
            }

            try {
                std::string encrypted_user_id = body["id"].s();
                std::string decrypted_user_id = AES.Decrypt(encrypted_user_id);
                std::string encrypted_friend_id = body["blocked_user_id"].s();
                std::string decrypted_friend_id = AES.Decrypt(encrypted_friend_id);
